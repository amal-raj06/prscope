"""
PRScope - Logistic Regression Pipeline (Small Dataset Optimized)
--------------------------------------------------------------
Uses L2-regularized Logistic Regression with balanced class weights 
to solve the micro-dataset splitting failures of LightGBM.
"""
...
def main():
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        # Crucial for linear models: Scale the features so large counts don't dominate
        scaler = StandardScaler()
        X_train_scaled = scaler.fit_transform(X_train)
        X_test_scaled = scaler.transform(X_test)

        # Logistic Regression handles small datasets vastly better than Trees
